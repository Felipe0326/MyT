...
export const KPI: React.FC<{ label: string; value: React.ReactNode; sub: React.ReactNode; color?: 'r' | 'g' | 'y' | 'm' | 'i' | 'e' }> = ({ label, value, sub, color }) => {
  const colorClasses = {
    r: 'text-rose-600',
    g: 'text-emerald-700',
    y: 'text-amber-600',
  ...
  return (
    <div className="bg-white p-6">
      ...
      <div className="text-xs text-stone-400 mt-2 leading-relaxed">{sub}</div>
    </div>
  );
};
