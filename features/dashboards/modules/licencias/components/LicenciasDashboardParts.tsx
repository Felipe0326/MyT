...
import type { LicenciaRecord, LicenciasResponse, LicenciasSortKey, LicenseYear, Modalidad, SortDirection } from "@/features/dashboards/modules/licencias/types";
...
export function DateField({ label, year, value, onChange }: { label: string; year: LicenseYear; value: string; onChange: (value: string) => void }) {
  return (
    <label className="block min-w-0 space-y-2 overflow-hidden">
      <span className="flex min-w-0 items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#74785C]">
        <CalendarDays className="h-3.5 w-3.5 shrink-0" /> {label}
      </span>
      <input
        type="date"
        min={`${year}-01-01`}
        ...
        className="block h-12 w-full min-w-0 max-w-full box-border rounded-xl border border-[#dedccf] bg-[#faf9f5] px-3 text-sm text-[#2E332A] outline-none focus:border-[#74785C] focus:ring-4 focus:ring-[#74785C]/10 sm:px-4"
      />
    </label>
  );
}
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-[26px] border border-[#dedccf] bg-white shadow-[0_10px_30px_rgba(46,51,42,0.07)]">
      <RecordsSectionHeader
        ...
        title="Registros diarios de Licencias"
        pageSize={pagination.pageSize}
        total={pagination.total}
        visible={visible}
        controlsId="licencias-records-content"
        exporting={exporting}
      ...
      {visible && (
        <div id="licencias-records-content">
